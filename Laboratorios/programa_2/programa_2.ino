...
rtc.adjust(DateTime(2022, 2, 3, 9, 30, 0));
DateTime fecha = rtc.now();        // funcion que devuelve fecha y horario en formato
              // DateTime y asigna a variable fecha
 if ( fecha.hour() == 12 && fecha.minute() == 0 && fecha.second() == 10){ // si hora = 14 y minutos = 30
   // if ( alarma == true ){        // si alarma tiene un valor verdadero
      
      Serial.println( "Alarma activada" ); 
      digitalWrite(13,HIGH);
      
      // imprime en monitor serie texto Alarma
 ...
 if ( fecha.hour() == 12 && fecha.minute() == 0 && fecha.second() == 20){ // si hora = 14 y minutos = 30
   // if ( alarma == true ){        // si alarma tiene un valor verdadero
